...
fn temperature_in_1_hour(
    ...
    iterations_count: Option<u8>,
    derivates: &[TemperatureDerivate],
) -> Template {
    ...
    let namespace_members: Vec<(Cow<'static, _>, _, _, _)> = [(
        Cow::Borrowed("t"),
    ...
    let namespace = template.assign_new_namespace(
        namespace_members
            .iter()
            .map(|(name, template, _mul, _loss)| (name.clone(), template.clone())),
    );
    let iterations_count = iterations_count.unwrap_or(12);
    let iterations = TemplateExpression::literal(iterations_count);
    let variables: Vec<_> = namespace_members
        .into_iter()
        .map(|(name, _, multiplier, loss)| {
            (
                Rc::from(namespace.member(name)),
                multiplier.map(TemplateExpression::literal),
                loss.map(|x| (1.0 - (x / f32::from(iterations_count)).max(0.0).min(1.0)))
    ...
    template.for_each(
        ...
        |template, _var| {
            for window in variables.windows(2) {
                let [(target, _, loss), (derivate, multiplier, _)] = window else {
                    unreachable!()
                };
                ...
                let mut new_value = &****target + &*change;
                if let Some(loss) = loss {
                    new_value = &*new_value * loss.clone();
                }
                template.assign(target, new_value);
            }
            if let Some((target, _, Some(loss))) = variables.last() {
                template.assign(target, &****target * loss.clone());
            }
        },
    );
    template.expr(
...
impl Package {
    fn build(
        ...
        let externals = if let Some(external_sensor) = &room.temperature_sensor {
            let external_temperature = external_sensor.clone().state();
        ...
        for radiator in &room.radiators[..] {
            room_current_temperature_entities
                .push(radiator.entity_id.clone().attribute("current_temperature"));
            room_chosen_temperature_entities
                .push(radiator.entity_id.clone().attribute("temperature"));
        }
        let entity_id = {
        ...
        if let Some((external_temperature, external_influence)) = externals {
            ...
            let weight = external_influence
                ...
                .to_int();
            let ext_mul = (&*weight.clone() / only_external.clone())
                .mark_named_const_expr("external_influence_percent");
        ...
        for (room_name, room) in config.rooms.iter() {
            ...
            let derivates_entities: Box<[_]> = {
                ...
                room.derivative_spannings
                    ...
                    .map(|(level, derivate)| {
                        ...
                        TemperatureDerivate {
                            entity: new,
                            multiplier: derivate.multiplier,
                            loss: derivate.loss,
                        }
                    })
                    .collect()
            ...
            let mut prediction_sensor = temperature_template_sensor(
                ...
                temperature_in_1_hour(
                    ...
                    room.derivative_iterations.or(config.derivative_iterations),
                    &derivates_entities,
                ),
            );
            let prediction_sources: Box<[_]> = [temperature_entity.clone()]
                .into_iter()
                .chain(derivates_entities.iter().map(|de| de.entity.clone()))
                .collect();
            ...
            for radiator in &room.radiators {
                ...
                let radiator_action =
                    add_automation_entity(radiator.entity_id.clone().attribute("hvac_action"));
                                    ...
                                    sequence: vec![Service::SetInputNumberValue {
                                        data: Service::template_data(
                                            max_closing_valve_entity
                                                .to_ha_call_named("max_closing_valve")
                                                .to_int(),
                                        ),
                                    ...
                                    sequence: vec![Service::SetInputNumberValue {
                                        data: Service::template_data(
                                            min_closing_valve_entity
                                                .to_ha_call_named("min_closing_valve")
                                                .to_int(),
                                        ),
                    ...
                    let log_previous = |expr: Rc<TemplateExpression>| {
                        Action::log(radiator.entity_id.clone(), |msg| {
                            ...
                            for derivate in &derivates_entities {
                                msg.expr(derivate.entity.entity_id().to_ha_call_pretty());
                                msg.text(", ");
                            }
                            msg.text("), tt=");
